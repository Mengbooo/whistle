...
function renderSectionMarkdown({ date, selected, bySection, sections, dropped, errors }) {
  const lines = [
    ...
    "",
    "这份 Markdown 是给 Agent 的素材，不是最终日报。最终日报标题必须是 `Whistle 互联网日报｜YYYY-MM-DD`。最终日报应按新版板块重写中文内容，并使用 Kami 生成 HTML。正文目标刊发 24 到 32 条；如果当天主信息源素材质量高，可以保留到 36 条。优先保留会影响产品、研发、增长、商业化、基础设施或合规的信息。不要因为话题热、争议大、媒体讨论多就放在前面。本日报不是把素材压缩成零散快讯，而是先保留来源事实骨架，再解释这件事是什么；每条 item 的正文应是一段自然中文，前半段交代原新闻的主体、动作、产品/模型/政策变化、关键数字、适用对象或限制条件，后半段解释为什么会影响行业、产品形态、工作方式、商业模式、分发渠道或监管边界。不要生成面向角色的行动建议，不要写“团队应该/可以/建议/需要”等执行口吻，也不要写“对产品、运营和设计团队”等岗位化表达。避免“今天的主线是、下沉、可执行工作流、反向约束、行业图景”等 AI 套话；如果条目具备趋势信号，请自然加入一句基于事实的影响说明，不要硬凑预测。",
    "",
    "## 今日头条候选",
    "",
  ...
  lines.push(
    "## 事件解释要求",
    "",
    "请 Agent 基于前面板块生成事实完整、但更容易读懂的解释。语言要自然、顺口、具体；不能变成口水话、情绪评论、观点输出或建议清单。只解释这件事是什么、为什么重要、它可能改变什么，不指挥读者下一步做什么，不按岗位分发影响。",
    "",
    "## 去噪说明",
    "",

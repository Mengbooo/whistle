...
function renderSectionMarkdown({ date, selected, bySection, sections, dropped, errors }) {
  const lines = [
    ...
    "",
    "这份 Markdown 是给 Agent 的素材，不是最终日报。最终日报标题必须是 `Whistle 互联网日报｜YYYY-MM-DD`。最终日报应按新版板块重写中文内容，并使用 Kami 生成 HTML。正文目标刊发 24 到 32 条；如果当天主信息源素材质量高，可以保留到 36 条。优先保留工作性信号：会影响产品、研发、增长、商业化、基础设施、合规或团队决策的信息。不要因为话题热、争议大、媒体讨论多就放在前面。本日报不是把素材压缩成零散快讯，而是先保留来源事实骨架，再给出编辑解读；每条 item 的正文应是一段自然中文，前半段交代原新闻的主体、动作、产品/模型/政策变化、关键数字、适用对象或限制条件，后半段解释这件事是什么、为什么会影响行业、产品形态、工作方式、商业模式、分发渠道或监管边界。不要生成面向角色的行动建议，不要写“团队应该/可以/建议/需要”等执行口吻；如果条目具备趋势信号，请自然加入一句未来判断或编辑评论，不要硬凑预测。",
    "",
    "## 今日头条候选",
    "",
  ...
  lines.push(
    "## 编辑解读要求",
    "",
    "请 Agent 基于前面板块生成事实完整、但更容易读懂的专业解读。解读可以更直白、更通俗，但不能变成口水话、情绪评论或建议清单；只解释这件事是什么、为什么重要、它可能改变什么，不指挥读者下一步做什么。",
    "",
    "## 去噪说明",
    "",

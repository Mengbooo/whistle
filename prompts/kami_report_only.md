...
- 每条核心内容的 `summary` 必须是一段自然中文，先保留来源事实，再给出编辑解读。通常用 2 到 5 句：前半段交代原新闻的主体、动作、产品/模型/政策变化、数字、适用对象或限制条件；后半段解释这件事到底是什么、为什么会影响行业、产品形态、工作方式、商业模式、分发渠道或监管边界。
...
- 不要在 `summary` 中写行动建议，不要使用“团队应该/可以/建议/需要/值得立刻/今日可以采取”等面向执行的措辞。只解释影响和含义，不指挥读者下一步做什么。
...
```json
{
  ...
  "summary": "一段 220 到 360 字的今日总述，纵览全文：今天发生了哪些主要事情，它们之间有什么相似性、共同趋势、反向约束和可能影响。写得可以更直白，但不要变成建议清单。",
